...
int main( int argc, char* argv[] ) { 
    ...
    cudaMalloc(&dev_a, vector_size*sizeof(int)); // HERE
    cudaMalloc(&dev_b, vector_size*sizeof(int));
    cudaMalloc(&dev_c, vector_size*sizeof(int));

    // copy the arrays 'a' and 'b' to the GPU
    cudaMemcpy(dev_a, a, vector_size*sizeof(int), cudaMemcpyHostToDevice);// HERE
    ...
    cudaMemcpy(c_gpu, dev_c, vector_size*sizeof(int), cudaMemcpyDeviceToHost); // HERE (there's one more at the end, don't miss it!)
